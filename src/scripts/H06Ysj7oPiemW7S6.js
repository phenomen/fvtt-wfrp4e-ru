if (args.test.options.income && !args.test.options.criminal)
	{
		...
		this.script.scriptMessage(`<a class="money-drag" data-amt="${moneyString}"><strong>Заработано дополнительно ${game.wfrp4e.market.amountToString(game.wfrp4e.market.parseMoneyTransactionString(transactionString))}</strong></a>`, {whisper : ChatMessage.getWhisperRecipients("GM")})
	}