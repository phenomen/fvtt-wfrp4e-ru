...
if (wounds.value > 0) {
	...
	message += `<b>${this.actor.name}</b> восстанавливает ${regen} пунктов здоровья.`;

	if (regen === 10) {
		message += "<br>Дополнительно регенрирует травму.";
	}
} else if (regen >= 8) {
	message += `<b>${this.actor.name}</b> совершил бросок ${regen} и восстанавливается 1 пункт здоровья.`;
	wounds.value += 1;
	if (regen === 10) {
		message += "<br>Дополнительно регенрирует травму.";
	}
} else {
	message += `<b>${this.actor.name}</b> Бросок регенерации ${regen} - нет эффекта.`;
}
